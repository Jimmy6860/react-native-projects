...
sauce.prototype.browsers = function(cb) {
  request.get({url:"https://saucelabs.com/rest/v1/info/browsers/webdriver", json: true}, function (e, resp) {
    cb(e, resp.body);
  })
};
        ...
        browser.init( desired, function() {
          browser.get(argv.url || "http://hello-internet.org", function() {
            var r = repl.start({prompt: '( '+argv.browser+' ): '
                                ...
                                , useColors: true
                                , safeEval: function(cmd, context, filename, callback) {
                                  if (cmd.length > 3) {
                                    browser.eval(cmd, function(err, out) {
                                      if (err) { callback(err.red + "\n"); }
                                      if (out) { callback(out.green + "\n"); }
                                    });
                                  }
                                }
                              });
